...
@Component
public class GridPoint16DaoImpl extends AbstractDao<GridPoint> implements GridPoint16Dao {
	private static final Double BOUNDING_BOX_MILES_PER_SIDE = 16.0;  
	...
	private static final String INSERT_SQL = "INSERT INTO grid_point_16 (lat, lon, closest_coastline_point_id, distance_in_miles) VALUES(?, ?, ?, ?)";
	private static final String UPDATE_SQL = "UPDATE grid_point_16 SET lat=?, lon=?, closest_coastline_point_id=?, distance_in_miles=? WHERE id=?";
	private static final String GET_BETWEEN_SQL = "SELECT * FROM grid_point_16 WHERE id BETWEEN ? AND ?";
	private static final String GET_OFFSET_SQL = "SELECT * FROM grid_point_16 ORDER BY id LIMIT ?, ?";
	private static final String GET_ALL_SQL = "SELECT * FROM grid_point_16";
	private static final String GET_POINTS_SURROUNDING_SQL = "SELECT * FROM grid_point_16 WHERE lat BETWEEN (? - " + BOUNDING_BOX_OFFSET_LAT + ") AND (? + " + BOUNDING_BOX_OFFSET_LAT + ") AND lon BETWEEN (? - " + BOUNDING_BOX_OFFSET_LON + ") AND (? + " + BOUNDING_BOX_OFFSET_LON + ")";
	
	@Autowired
	private DataSource datasource;
	
	public GridPoint16DaoImpl() {
