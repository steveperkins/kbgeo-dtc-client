...
//	private static final String GET_BETWEEN_SQL = "SELECT * FROM coastline_point WHERE client_id=? AND sort_order BETWEEN ? AND ?";
	private static final String GET_BETWEEN_SQL = "SELECT cp.* FROM coastline_point cp INNER JOIN kbs_client kc ON kc.id=? WHERE client_id=kc.coastline_client_id AND sort_order BETWEEN ? AND ?";
	private static final String GET_BETWEEN_SORT_ORDERS_SQL = "WITH segment_override AS (" 
			  + "SELECT DISTINCT segment_id FROM client_coastline_segment INNER JOIN kbs_client kc ON kc.id=? WHERE client_id=COALESCE(kc.coastline_client_id, 1) AND active = TRUE"
			  + ") "
			  + "SELECT cp.* "
			  + "FROM coastline_point cp " 
			  + "INNER JOIN kbs_client kc ON kc.id=? "
			  + "LEFT JOIN segment_override so ON cp.segment_id=so.segment_id " 
			  + "WHERE cp.sort_order BETWEEN ? AND ? AND ((so.segment_id IS NULL AND cp.client_id=1) OR cp.client_id=COALESCE(kc.coastline_client_id, 1))";
	private static final String GET_ALL_SQL = "SELECT cp.* FROM coastline_point cp INNER JOIN kbs_client kc ON kc.id=? WHERE cp.client_id=COALESCE(kc.coastline_client_id, 1)";
	private static final String GET_BY_COAST_SQL = "SELECT cp.* FROM coastline_point cp INNER JOIN kbs_client kc ON kc.id=? WHERE cp.client_id=COALESCE(kc.coastline_client_id, 1) AND coast=?";
	private static final String GET_ALL_POINTS_BY_CLIENT_SEGMENT_SQL = "SELECT cp.* FROM coastline_point cp INNER JOIN kbs_client kc ON kc.id=? WHERE cp.client_id=COALESCE(kc.coastline_client_id, 1) AND cp.segment_id=?";
	private static final String GET_POINTS_BEFORE_AND_AFTER_POINT = "(SELECT cp.* FROM coastline_point cp INNER JOIN kbs_client kc ON kc.id=? WHERE cp.client_id=COALESCE(kc.coastline_client_id, 1) AND cp.sort_order < ? ORDER BY cp.sort_order DESC LIMIT 1) UNION (SELECT cp.* FROM coastline_point cp INNER JOIN kbs_client kc ON kc.id=? WHERE cp.client_id=COALESCE(kc.coastline_client_id, 1) AND cp.sort_order > ? ORDER BY cp.sort_order LIMIT 1)";
	...
	public List<CoastlinePoint> getPointsBetweenSortOrders(Integer clientId, Double startOrder, Double endOrder) {
		List<CoastlinePoint> points = jdbcTemplate.query(GET_BETWEEN_SORT_ORDERS_SQL, new Number[]{ clientId, clientId, startOrder, endOrder }, new CoastlinePointRowMapper());
		return (null == points ? new ArrayList<CoastlinePoint>() : points);
	}
