...
@Component
public class GridPoint64DaoImpl extends AbstractDao<GridPoint> implements GridPoint64Dao {
	private static final Double BOUNDING_BOX_MILES_PER_SIDE = 64.0;  
	...
	private static final String INSERT_SQL = "INSERT INTO grid_point_64 (lat, lon, closest_coastline_point_id, distance_in_miles) VALUES(?, ?, ?, ?)";
	private static final String UPDATE_SQL = "UPDATE grid_point_64 SET lat=?, lon=?, closest_coastline_point_id=?, distance_in_miles=? WHERE id=?";
	private static final String GET_BETWEEN_SQL = "SELECT * FROM grid_point_64 WHERE id BETWEEN ? AND ?";
	private static final String GET_OFFSET_SQL = "SELECT * FROM grid_point_64 ORDER BY id LIMIT ?, ?";
	private static final String GET_ALL_SQL = "SELECT * FROM grid_point_64";
	private static final String GET_POINTS_SURROUNDING_SQL = "SELECT * FROM grid_point_64 WHERE lat BETWEEN (? - " + BOUNDING_BOX_OFFSET_LAT + ") AND (? + " + BOUNDING_BOX_OFFSET_LAT + ") AND lon BETWEEN (? - " + BOUNDING_BOX_OFFSET_LON + ") AND (? + " + BOUNDING_BOX_OFFSET_LON + ")";
	
	@Autowired
	private DataSource datasource;
	
	public GridPoint64DaoImpl() {
