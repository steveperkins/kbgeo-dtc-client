...
@Component
public class GridPoint128DaoImpl extends AbstractDao<GridPoint> implements GridPoint128Dao {
	...
	private static final String INSERT_SQL = "INSERT INTO grid_point_128 (lat, lon) VALUES(?, ?)";
	private static final String UPDATE_SQL = "UPDATE grid_point_128 SET lat=?, lon=?, closest_coastline_point_id=?, distance_in_miles=? WHERE id=?";
	private static final String GET_BETWEEN_SQL = "SELECT * FROM grid_point_128 WHERE id BETWEEN ? AND ?";
	private static final String GET_OFFSET_SQL = "SELECT * FROM grid_point_128 where closest_coastline_point_id is null ORDER BY id LIMIT ?, ?";
	private static final String GET_ALL_SQL = "SELECT * FROM grid_point_128";
	private static final String GET_POINTS_SURROUNDING_SQL = "SELECT * FROM grid_point_128 WHERE lat BETWEEN (? - " + BOUNDING_BOX_OFFSET_LAT + ") AND (? + " + BOUNDING_BOX_OFFSET_LAT + ") AND lon BETWEEN (? - " + BOUNDING_BOX_OFFSET_LON + ") AND (? + " + BOUNDING_BOX_OFFSET_LON + ")";
	
	@Autowired
	private DataSource datasource;
	
	public GridPoint128DaoImpl() {
	...
	protected int[] getUpdateParamTypes() {
		return new int[] { 
				Types.DECIMAL, // lat
				Types.DECIMAL, // lon
				Types.BIGINT, // closest_coastline_point_id
